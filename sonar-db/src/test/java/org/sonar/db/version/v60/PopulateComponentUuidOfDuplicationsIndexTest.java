...
public class PopulateComponentUuidOfDuplicationsIndexTest {
  ...
  public void migration_updates_component_uuid_with_values_from_table_snapshots_when_they_exist() throws SQLException {
    ...
    insertDuplicationIndex(3, 90); // 90 does not exist
    insertDuplicationIndex(4, 100); // 100 does not exist
    db.commit();

    underTest.execute();

    verifyDuplicationsIndex(1, 40, uuid1);
    verifyDuplicationsIndex(2, 60, uuid3);
    verifyDuplicationsIndex(3, 90, null);
    verifyDuplicationsIndex(4, 100, null);
  }
