  ...
  def has_role_for_resources?(user, role, component_uuids)
    ...
    if group_ids.empty?
      ...
      page_count.times do |page_index|
        ...
        group_roles.concat(
          ActiveRecord::Base.connection.execute("SELECT p.uuid FROM group_roles gr INNER JOIN projects p ON p.id=gr.resource_id WHERE gr.role='#{sanitized_role}' and gr.group_id is null and p.uuid in (#{page_component_uuids.join(',')})")
        )
      end
    else
      ...
      page_count.times do |page_index|
        ...
        gr_page_count.times do |gr_page_index|
          ...
          group_roles.concat(
            ActiveRecord::Base.connection.execute("SELECT p.uuid FROM group_roles gr INNER JOIN projects p ON p.id=gr.resource_id WHERE gr.role='#{sanitized_role}' and (gr.group_id is null or gr.group_id in(#{page_grids.join(',')})) and p.uuid in (#{page_component_uuids.join(',')})")
          )
        end
      end
    end
    ...
    if user.id
      page_count.times do |page_index|
        page_component_uuids=compacted_component_uuids[page_index*page_size...(page_index+1)*page_size]
        user_roles.concat(
          ActiveRecord::Base.connection.execute("SELECT p.uuid as 'uuid' FROM user_roles ur INNER JOIN projects p ON p.id=ur.resource_id WHERE ur.role='#{sanitized_role}' and ur.user_id=#{user.id} and p.uuid in (#{page_component_uuids.join(',')})")
        )
      end
    end
