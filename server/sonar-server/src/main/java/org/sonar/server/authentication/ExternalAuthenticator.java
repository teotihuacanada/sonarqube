...
public class ExternalAuthenticator implements Startable {
  ...
  private Authenticator authenticator = realm.doGetAuthenticator();
  private ExternalUsersProvider externalUsersProvider;
  private ExternalGroupsProvider externalGroupsProvider;

  public ExternalAuthenticator(Settings settings, SecurityRealmFactory securityRealmFactory, UserAuthenticator userAuthenticator) {
  ...
  private UserDto synchronize(String userLogin, UserDetails details, HttpServletRequest request) {
    ...
    return userAuthenticator.register(userIdentityBuilder.build(), new ExternalIdentityProvider());
  }
